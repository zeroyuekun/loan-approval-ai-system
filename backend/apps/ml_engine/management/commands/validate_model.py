...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--challenger', type=str, default=None,
            help='UUID of a specific challenger ModelVersion (default: all inactive)',
        )
        parser.add_argument(
            '--validator', type=str, default='System (automated)',
            help='Name of the validator performing this review',
        )
        parser.add_argument(
            '--role', type=str, default='Automated Validation Pipeline',
    ...
    def handle(self, *args, **options):
        ...
        if options['challenger']:
            ...
            challengers = ModelVersion.objects.filter(
                is_active=False, retired_at__isnull=True,
            ).exclude(pk=champion.pk).order_by('-created_at')[:3]
        ...
        for challenger in challengers:
            try:
                ...
                self.stdout.write(self.style.WARNING(
                    f'Could not evaluate challenger {challenger.version}: {e}'
                ))
        ...
        findings_parts.append(
            f'Champion model (v{champion.version}) evaluated on {len(df)} holdout samples.'
        )
        findings_parts.append(
            f'AUC-ROC: {champion_metrics["auc_roc"]:.4f}, '
            f'Brier: {champion_metrics.get("brier_score", "N/A")}'
        )
        ...
        for attr, data in fairness.items():
            if isinstance(data, dict) and data.get('disparate_impact_ratio', 1.0) < 0.80:
                fairness_issues.append(
                    f'{attr}: DI ratio = {data["disparate_impact_ratio"]:.3f}'
                )
