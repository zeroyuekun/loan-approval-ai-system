...
def impute_missing_values(df, imputation_values=None):
    ...
    priority_cols = ["monthly_expenses", "existing_credit_card_limit", "property_value", "deposit_amount"]
...
def compute_derived_features(df):
    ...
    df["income_credit_interaction"] = np.log1p(df["annual_income"]) * df["credit_score"] / 1200

    # Serviceability buffer
    monthly_commitments = df["existing_credit_card_limit"] * 0.03 + df["monthly_expenses"]
    ...
    emp_type_weight = (
        df.get("employment_type", pd.Series(dtype=str))
        .map(
            {
                "payg_permanent": 1.0,
                "contract": 0.7,
    ...
    estimated_monthly = np.where(
        term > 0,
        df["loan_amount"] * assessment_rate * (1 + assessment_rate) ** term / ((1 + assessment_rate) ** term - 1),
        0.0,
    )
    ...
    df["net_monthly_surplus"] = np.where(
        monthly_income.notna() & (monthly_income > 0),
        np.clip(
            (monthly_income - df["monthly_expenses"] - df["existing_credit_card_limit"] * 0.03 - estimated_monthly)
            / monthly_income,
            -2.0,
            1.0,
        ),
    ...
    df["enquiry_intensity"] = df["num_credit_enquiries_6m"] / np.maximum(df["credit_history_months"] / 12, 1)
    ...
    df["rate_stress_buffer"] = (
        df["rba_cash_rate"] / 100 * df["loan_amount"] / 12 / np.maximum(df["annual_income"] / 12, 1)
    )
    ...
    df["stressed_repayment"] = np.where(
        term_s > 0,
        df["loan_amount"] * stressed_rate * (1 + stressed_rate) ** term_s / ((1 + stressed_rate) ** term_s - 1),
        0.0,
    )
    ...
    df["stress_index"] = np.clip(
        utilization * 30 + neg_days * 1.5 + overdraft * 3 + df["stressed_dsr"] * 40,
        0,
        100,
    )
