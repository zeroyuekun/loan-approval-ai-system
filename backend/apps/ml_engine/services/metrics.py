...
class MetricsService:
    ...
    def compute_all_woe_iv(self, df, y_true, numeric_cols, n_bins=10):
        ...
        for col in numeric_cols:
            if col in df.columns:
                result = self.compute_woe_iv(df[col].values, y_true, n_bins=n_bins, feature_name=col)
                results[col] = result

        # Sort by IV descending
        sorted_results = dict(sorted(results.items(), key=lambda x: x[1]["iv"], reverse=True))
    ...
    def build_woe_scorecard(self, X_train, y_train, numeric_cols, n_bins=10, X_test=None, y_test=None):
        ...
        for col in numeric_cols:
            ...
            result = self.compute_woe_iv(X_train[col].values, y_train.values, n_bins=n_bins, feature_name=col)
            ...
            bin_indices = np.clip(np.digitize(X_train[col].values, bin_edges[1:-1]), 0, len(woe_values) - 1)
    ...
    def compute_expected_loss(self, pd_value, loan_amount, purpose, lvr=0.0, credit_score=864):
...
class VintageAnalyser:
    ...
    def compute_survival_metrics(df: pd.DataFrame) -> dict:
        ...
        for t in range(1, max_mob + 1):
            ...
            events = len(df[(df["months_on_book"] >= t) & (df["default_flag"] == 1) & (df["months_on_book"] < t + 1)])
    ...
    def compute_temporal_psi(
        df: pd.DataFrame,
        score_col: str = "prediction_probability",
