...
class CalibrationValidator:
    ...
    def validate_prediction_calibration(
        ...
        logger.info(
            'Calibration validation: overall=%s, internal=%s (gap=%.4f), '
            'external=%s (gap=%.4f), n=%d',
            overall_status, internal_status, internal_gap,
            external_status, external_gap, total_applications,
        )
        return result
    ...
    def validate_by_state(
        ...
        for state, outcomes in state_outcomes.items():
            ...
            if apra_rate is None:
                logger.warning(
                    'No APRA benchmark for state %s — skipping', state_upper,
                )
                results[state_upper] = {
            ...
            if status != 'aligned':
                logger.info(
                    'State %s calibration: system=%.4f vs APRA=%.4f '
                    '(gap=%.4f, n=%d, APRA %s)',
                    state_upper, system_rate, apra_rate, gap, count,
                    apra_quarter,
                )
    ...
    def validate_portfolio_composition(
        ...
        logger.info(
            ...
            lvr_status, lvr_gap, dti_status, dti_gap,
        )
        return result
    ...
    def generate_calibration_report(
        ...
        all_recommendations = list(
            report['prediction_calibration'].get('recommendations', [])
        )
        ...
        if 'portfolio_composition' in report:
            all_recommendations.extend(
                report['portfolio_composition'].get('recommendations', [])
            )
        ...
        logger.info(
            ...
            overall, len(all_recommendations), total_applications,
        )
        return report
