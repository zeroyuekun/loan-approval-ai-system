...
_FALLBACK_INCOME_PERCENTILES = {
    ...
    'national': {
        'P10': 25_000, 'P25': 42_000, 'P50': 74_100,
        'P75': 105_000, 'P90': 155_000,
...
class RealWorldBenchmarks:
    ...
    def get_income_percentiles(self, state: str = 'national') -> dict:
        ...
        """
        fallback = _FALLBACK_INCOME_PERCENTILES.get(
            state, _FALLBACK_INCOME_PERCENTILES['national']
        )
    ...
    def get_calibration_snapshot(self) -> dict:
        ...
        snapshot = {
            ...
            'income_percentiles_by_state': {
                s: self.get_income_percentiles(s)
                for s in ['NSW', 'VIC', 'QLD', 'WA', 'SA', 'TAS', 'ACT', 'NT']
            },
        ...
        logger.info(
            ...
            {k: v.get('source', 'n/a') if isinstance(v, dict) else 'composite'
    ...
    def get_cumulative_default_prob(cls, months_on_book: int) -> float:
        ...
        for i in range(len(breakpoints) - 1):
            if breakpoints[i] <= months_on_book <= breakpoints[i+1]:
                ...
                return cls.DEFAULT_HAZARD_CURVE[lo] + frac * (cls.DEFAULT_HAZARD_CURVE[hi] - cls.DEFAULT_HAZARD_CURVE[lo])
        return cls.DEFAULT_HAZARD_CURVE[breakpoints[-1]]
