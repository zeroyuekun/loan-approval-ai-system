...
def test_full_flow_file_then_overturn(django_user_model, monkeypatch):
    ...
    cclient = APIClient(); cclient.force_authenticate(cust)
    r = cclient.post("/api/v1/loans/decision-reviews/",
                     {"application": str(app.id), "reason": "My income was understated"}, format="json")
    assert r.status_code == 201
    review_id = r.json()["id"]
    assert AuditLog.objects.filter(action="decision_review_requested", resource_id=review_id).exists()

    # 2. Officer overturns it
    oclient = APIClient(); oclient.force_authenticate(officer)
    r2 = oclient.post(f"/api/v1/loans/decision-reviews/{review_id}/resolve/",
                      {"outcome": "overturned", "note": "Verified income — approving"}, format="json")
...
def test_full_flow_file_then_uphold_keeps_denied(django_user_model):
    cust, officer, app = _denied(django_user_model)
    cclient = APIClient(); cclient.force_authenticate(cust)
    review_id = cclient.post("/api/v1/loans/decision-reviews/",
                             {"application": str(app.id), "reason": "please re-check"}, format="json").json()["id"]
    oclient = APIClient(); oclient.force_authenticate(officer)
    r = oclient.post(f"/api/v1/loans/decision-reviews/{review_id}/resolve/",
                     {"outcome": "upheld", "note": "Decision confirmed"}, format="json")
