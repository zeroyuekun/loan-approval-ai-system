...
class LoanApplication(SoftDeleteModel):
    ...
    worst_arrears_months = models.IntegerField(null=True, blank=True, help_text='Worst arrears in last 24 months (0-3+)')
    ...
    savings_balance = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True, help_text='Average savings balance')
    ...
    unemployment_rate = models.FloatField(null=True, blank=True, help_text='State unemployment rate at application date')
    ...
    savings_trend_3m = models.CharField(max_length=10, null=True, blank=True, choices=[('positive', 'Positive'), ('negative', 'Negative'), ('flat', 'Flat')], help_text='3-month savings trend')
    discretionary_spend_ratio = models.FloatField(null=True, blank=True, help_text='Ratio of discretionary to essential spending')
    ...
    income_verification_score = models.FloatField(null=True, blank=True, help_text='0.0-1.0: declared vs observed income consistency')

    # CCR (Comprehensive Credit Reporting) — mandatory since 2018
    num_late_payments_24m = models.IntegerField(null=True, blank=True, default=0, help_text='Late (14+ days) payments in 24-month CCR window')
    worst_late_payment_days = models.IntegerField(null=True, blank=True, default=0, help_text='Worst arrears: 0, 14, 30, 60, 90+ days')
    total_credit_limit = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True, help_text='Sum of all credit limits from CCR')
    credit_utilization_pct = models.FloatField(null=True, blank=True, help_text='Total balance / total credit limit')
    num_hardship_flags = models.IntegerField(null=True, blank=True, default=0, help_text='Financial Hardship Information flags (CCR since 2022)')
    months_since_last_default = models.IntegerField(null=True, blank=True, help_text='Months since most recent default')
    num_credit_providers = models.IntegerField(null=True, blank=True, default=1, help_text='Count of distinct credit providers')
    # BNPL-specific (NCCP Act since June 2025)
    bnpl_total_limit = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, default=0, help_text='Total BNPL credit limit')
    bnpl_utilization_pct = models.FloatField(null=True, blank=True, default=0, help_text='BNPL balance / BNPL limit')
    bnpl_late_payments_12m = models.IntegerField(null=True, blank=True, default=0, help_text='Late BNPL payments in 12 months')
    bnpl_monthly_commitment = models.DecimalField(max_digits=8, decimal_places=2, null=True, blank=True, default=0, help_text='Average monthly BNPL repayment')
    # CDR/Open Banking transaction features (Consumer Data Right)
    income_source_count = models.IntegerField(null=True, blank=True, default=1, help_text='Count of distinct income sources')
    rent_payment_regularity = models.FloatField(null=True, blank=True, help_text='0-1: on-time rent payment consistency')
    utility_payment_regularity = models.FloatField(null=True, blank=True, help_text='0-1: on-time utility payment consistency')
    essential_to_total_spend = models.FloatField(null=True, blank=True, help_text='Essential spending / total spending')
    subscription_burden = models.FloatField(null=True, blank=True, help_text='Total recurring subscriptions / monthly income')
    balance_before_payday = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, help_text='Average balance 3 days before salary')
    min_balance_30d = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, help_text='Lowest account balance in last 30 days')
    days_negative_balance_90d = models.IntegerField(null=True, blank=True, default=0, help_text='Days with negative balance in 90 days')
    # Geographic risk
    postcode_default_rate = models.FloatField(null=True, blank=True, help_text='Historical default rate for postcode area')
    industry_risk_tier = models.CharField(max_length=20, null=True, blank=True, choices=[('low', 'Low'), ('medium', 'Medium'), ('high', 'High'), ('very_high', 'Very High')])
    ...
    actual_outcome = models.CharField(max_length=20, null=True, blank=True, choices=[
        ('performing', 'Performing'), ('arrears_30', '30-Day Arrears'),
        ('arrears_60', '60-Day Arrears'), ('arrears_90', '90+ Day Arrears'),
        ('default', 'Default'), ('prepaid', 'Prepaid Early'),
    ])
...
class FraudCheck(models.Model):
    ...
    application = models.ForeignKey(
        LoanApplication, on_delete=models.CASCADE, related_name='fraud_checks'
    )
