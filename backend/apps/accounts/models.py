...
class CustomerProfile(SoftDeleteModel):
    ...
    date_of_birth = EncryptedCharField(
        max_length=500, blank=True, default="", help_text="ISO-8601 date string, encrypted at rest"
    )
    ...
    previous_employer = models.CharField(
        max_length=255, blank=True, default="", help_text="Required if less than 2 years in current role"
    )

    # ── Income (encrypted at rest — stored as string representations) ──
    gross_annual_income = EncryptedCharField(
        max_length=500, blank=True, default="", help_text="Decimal string, encrypted at rest"
    )
    other_income = EncryptedCharField(
        max_length=500, blank=True, default="0", help_text="Decimal string, encrypted at rest"
    )
    other_income_source = models.CharField(max_length=100, blank=True, default="")
    partner_annual_income = EncryptedCharField(
        max_length=500, blank=True, default="", help_text="Decimal string, encrypted at rest"
    )
    ...
    privacy_consent_given = models.BooleanField(
        default=False, help_text="Customer consented to Privacy Collection Notice"
    )
