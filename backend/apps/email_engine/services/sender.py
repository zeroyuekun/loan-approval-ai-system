...
SECTION_LABELS = [
    'Loan Details:', 'Next Steps:', 'Required Documentation:',
    'Before You Sign:', "We're Here For You:", 'What You Can Do:',
    "We'd Still Like to Help:", 'Attachments:', 'Conditions of Approval:',
...
def _plain_text_to_html(body: str) -> str:
    ...
    for line in lines:
        ...
        if is_section or is_option:
            _flush_detail_rows()
            html_parts.append(
                f'<p style="margin:20px 0 4px 0;"><strong>{stripped}</strong></p>'
            )
            continue
        ...
        if is_dear:
            _flush_detail_rows()
            html_parts.append(
                f'<p style="margin:0 0 4px 0;"><strong>{stripped}</strong></p>'
            )
            continue
        ...
        if bullet_match:
            ...
            if 0 < colon_idx < 30:
                ...
                if value:
                    detail_rows.append(
                        f'<tr><td {td_label}>&nbsp;&nbsp;{label}:</td>'
                        f'<td {td_value}>{value}</td></tr>'
                    )
                    continue
            html_parts.append(
                f'<p style="margin:2px 0 2px 16px;">• {content}</p>'
            )
            continue
        ...
        if stripped.startswith('ABN ') or stripped.startswith('Ph:') or stripped.startswith('Phone:') or stripped.startswith('Email:') or stripped.startswith('Website:'):
            html_parts.append(
        ...
        html_parts.append(
            f'<p style="margin:{top_margin} 0 {margin} 0;">{stripped}</p>'
        )
