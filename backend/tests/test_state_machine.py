...
class TestLoanStatusTransitions(TestCase):
    ...
    def test_initial_status_is_pending(self):
        ...
        assert self.app.status == 'pending', \
            f'Initial status should be pending, got {self.app.status}'
    ...
    def test_valid_status_choices(self):
        ...
        assert valid_statuses == expected, \
            f'Status choices mismatch: got {valid_statuses}, expected {expected}'
    ...
    def test_status_persists_after_save(self):
        ...
        for target_status in ['processing', 'approved', 'conditional', 'denied', 'review']:
            ...
            self.app.save()
            self.app.refresh_from_db()
            assert self.app.status == target_status, \
                f'Status {target_status} did not persist after save'
