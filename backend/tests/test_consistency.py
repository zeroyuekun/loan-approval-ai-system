...
class TestDepositVsProperty:
    def test_deposit_exceeds_property_error(self, checker):
        ...
        result = checker.check_all(features)
        assert result["consistent"] is False
        assert any("deposit" in e["message"].lower() and "property" in e["message"].lower() for e in result["errors"])
    ...
    def test_non_home_skips_deposit_vs_property(self, checker):
        ...
        assert not any(
            "deposit" in e["message"].lower() and "property" in e["message"].lower() for e in result["errors"]
        )
...
class TestDepositVsLoan:
    def test_deposit_much_larger_than_loan_warning(self, checker):
        ...
        result = checker.check_all(features)
        assert any("deposit" in w["message"].lower() and "loan" in w["message"].lower() for w in result["warnings"])
    ...
    def test_deposit_within_threshold_no_warning(self, checker):
        ...
        assert not any(
            "deposit" in w["message"].lower() and "significantly larger" in w["message"].lower()
...
class TestLvrSanity:
    def test_lvr_above_100_error(self, checker):
        ...
        assert result["consistent"] is False
        assert any(
            "lvr" in e["message"].lower() or "exceeds property" in e["message"].lower() for e in result["errors"]
        )
...
class TestExpensesVsIncome:
    ...
    def test_very_high_expenses_warning(self, checker):
        ...
        result = checker.check_all(features)
        assert any("expenses" in w["message"].lower() and "consume" in w["message"].lower() for w in result["warnings"])
