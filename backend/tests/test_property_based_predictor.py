...
def _make_fake_bundle(feature_cols=None):
    ...
    if feature_cols is None:
        ...
        df = pd.get_dummies(df, columns=[
            'purpose', 'home_ownership', 'employment_type',
            'applicant_type', 'state', 'savings_trend_3m', 'industry_risk_tier',
        ], dtype=float)
...
class TestFeatureBoundsValidation:
    ...
    def test_in_bounds_values_never_raise(self, features):
        ...
        with patch('apps.ml_engine.services.predictor._load_bundle', return_value=bundle), \
...
class TestRiskGradeMapping:
    ...
    def test_always_returns_valid_grade(self, prob):
        ...
        assert grade in VALID_RISK_GRADES, (
            f"compute_risk_grade({prob}) returned '{grade}', "
            f"which is not in {VALID_RISK_GRADES}"
        )
    ...
    def test_monotonicity(self, prob):
        ...
        assert grade_rank[g2] >= grade_rank[g1], (
            f"Monotonicity violated: prob={prob} -> {g1}, "
            f"prob={higher_prob} -> {g2}"
        )
...
class TestPredictionOutputShape:
    ...
    REQUIRED_KEYS = {
        'prediction', 'probability', 'risk_grade', 'feature_importances',
    }
...
_fe_base_strategy = st.fixed_dictionaries({
    ...
    'existing_credit_card_limit': st.floats(min_value=0.0, max_value=500_000.0, allow_nan=False, allow_infinity=False),
...
class TestFeatureEngineeringFiniteness:
    ...
    def test_derived_features_always_finite(self, base):
        ...
        base['employment_type'] = np.random.choice(
            ['payg_permanent', 'contract', 'self_employed', 'payg_casual']
        )
...
class TestEdgeCaseFeatures:
    ...
    def test_zero_income(self):
        ...
        for col in ['loan_to_income', 'credit_card_burden', 'expense_to_income',
                     'serviceability_ratio', 'monthly_repayment_ratio',
                     'net_monthly_surplus', 'stressed_dsr', 'bnpl_to_income_ratio']:
            if col in result.columns:
                val = result[col].iloc[0]
    ...
    def test_all_categorical_combinations_valid(self, purpose, home, emp, app_type, state):
        ...
        for col in numeric_cols:
            val = result[col].iloc[0]
            assert np.isfinite(val), (
                f"Categorical combo ({purpose},{home},{emp},{app_type},{state}): "
                f"'{col}' is not finite: {val}"
            )
