...
class TestFullPipeline(TestCase):
    ...
    def _create_application(self):
        ...
        assert resp.status_code == status.HTTP_201_CREATED, f"Application creation failed: {resp.data}"
        return resp.data
    ...
    def test_trigger_prediction(self, mock_delay):
        ...
        assert resp.status_code == status.HTTP_202_ACCEPTED, f"Prediction trigger: {resp.status_code} {resp.data}"
    ...
    def test_model_version_list_accessible_by_admin(self):
        ...
        assert resp.status_code in (200, 404), f"Unexpected status {resp.status_code} for model list"
