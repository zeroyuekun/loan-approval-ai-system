...
class TestComputeDerivedFeatures:
    ...
    def test_zero_income_no_division_error(self, complete_feature_row):
        ...
        for col in DERIVED_FEATURE_NAMES:
            ...
            assert np.isfinite(val), (
                f"Derived feature '{col}' is not finite ({val}) when income=0"
            )
    ...
    def test_home_loan_lvr_correct(self, complete_feature_row):
        ...
        row = {**complete_feature_row, 'loan_amount': 550000.0,
               'property_value': 700000.0}
        df = pd.DataFrame([row])
        result = compute_derived_features(df)
    ...
    @pytest.mark.parametrize('emp_type,expected_weight', [
        ('payg_permanent', 1.0),
        ('contract', 0.7),
        ('self_employed', 0.6),
        ('payg_casual', 0.4),
    ])
    def test_employment_stability_weights(self, complete_feature_row,
        ...
        row = {**complete_feature_row,
               'employment_type': emp_type,
               'employment_length': 5}
        df = pd.DataFrame([row])
        result = compute_derived_features(df)
        expected = expected_weight * np.log1p(5)
        assert result['employment_stability'].iloc[0] == pytest.approx(
            expected, rel=1e-6
        )
    ...
    def test_bureau_risk_score_formula(self, complete_feature_row):
        ...
        assert result['bureau_risk_score'].iloc[0] == pytest.approx(
            expected, rel=1e-9
        )
...
class TestBatchConsistency:
    ...
    def test_batch_500_rows_consistent(self):
        ...
        for col in DERIVED_FEATURE_NAMES:
            ...
            assert nan_count == 0, (
                f"Derived column '{col}' has {nan_count} NaN values in "
                f"500-row batch"
            )
            inf_count = np.isinf(result[col]).sum()
            assert inf_count == 0, (
                f"Derived column '{col}' has {inf_count} inf values in "
                f"500-row batch"
            )
