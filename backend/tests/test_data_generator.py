...
class TestDataGenerator:
    ...
    def test_has_all_expected_columns(self, df):
        expected_columns = [
            ...
            'has_cosigner', 'property_value', 'deposit_amount', 'monthly_expenses',
            'existing_credit_card_limit', 'number_of_dependants', 'employment_type',
            'applicant_type', 'has_hecs', 'has_bankruptcy', 'state',
            'application_quarter', 'rba_cash_rate', 'unemployment_rate',
            'property_growth_12m', 'consumer_confidence',
            ...
            'is_fraud_signal', 'income_verification_gap', 'address_tenure_months',
            'document_consistency_score',
            'approved', 'default_probability',
        ]
    ...
    def test_approval_rate_in_realistic_range(self, df):
        ...
        assert 0.50 <= approval_rate <= 0.75, (
            f"Approval rate {approval_rate:.2%} outside expected 50-75% range"
        )

    def test_default_probability_in_realistic_range(self, df):
        mean_default = df['default_probability'].mean()
        assert 0.02 <= mean_default <= 0.06, (
            f"Mean default probability {mean_default:.3f} outside expected 2-6% range"
        )
    ...
    def test_state_distribution_matches_population(self, df):
        ...
        assert state_counts.index[0] == 'NSW', (
            f"Expected NSW as largest, got {state_counts.index[0]}"
        )
    ...
    def test_behavioural_features_nan_for_non_customers(self, df):
        ...
        assert non_customers['savings_balance'].isna().all(), (
            "Non-existing customers should have NaN savings_balance"
        )
    ...
    def test_fraud_rate_approximately_2_percent(self, df):
        ...
        assert 0.005 <= fraud_rate <= 0.05, (
            f"Fraud rate {fraud_rate:.3f} outside expected 0.5-5% range"
        )
    ...
    def test_bureau_missing_for_non_customers(self, df):
        ...
        bureau_cols = ['num_credit_enquiries_6m', 'worst_arrears_months',
                       'num_defaults_5yr', 'total_open_accounts']
    ...
    def test_mnar_missing_correlated_with_credit(self, df):
        ...
        assert low_miss >= high_miss * 0.8, (
            f'MNAR expected: low credit missing {low_miss:.2%} should be >= '
            f'high credit missing {high_miss:.2%}'
        )
    ...
    def test_seasonal_quarter_distribution(self, generator):
        ...
        assert q4_total > q2_total, (
            f'Q4 total ({q4_total}) should exceed Q2 total ({q2_total})'
        )
...
class TestRealWorldDistributions:
    ...
    def test_approval_rate_matches_apra(self, generated_data):
        ...
        assert 0.45 <= approval_rate <= 0.70, \
            f'Approval rate {approval_rate:.1%} outside APRA range (45-70%)'
    ...
    def test_mean_income_realistic(self, generated_data):
        ...
        assert 50000 <= mean_income <= 180000, \
            f'Mean income ${mean_income:,.0f} outside realistic applicant range'
    ...
    def test_credit_score_distribution(self, generated_data):
        ...
        assert 750 <= mean_score <= 950, \
            f'Mean credit score {mean_score:.0f} outside realistic range (750-950)'
    ...
    def test_open_banking_features_present(self, generated_data):
        ...
        expected_cols = [
            'salary_credit_regularity', 'savings_trend_3m',
    ...
    def test_gambling_flag_prevalence(self, generated_data):
        ...
        if 'gambling_transaction_flag' in generated_data.columns:
            ...
            assert 0.05 <= rate <= 0.30, \
                f'Gambling flag rate {rate:.1%} outside expected range (5-30%)'
    ...
    def test_dti_range_realistic(self, generated_data):
        ...
        assert 1.0 <= mean_dti <= 6.0, \
            f'Mean DTI {mean_dti:.2f} outside realistic range (1.0-6.0)'
    ...
    def test_state_proportions_population_weighted(self, generated_data):
        ...
        assert 0.45 <= nsw_vic <= 0.70, \
            f'NSW+VIC proportion {nsw_vic:.1%} outside expected range (45-70%)'
...
class TestAustralianFeatures:
    ...
    def test_bnpl_limit_correlated_with_accounts(self, generated_data):
        ...
        if len(generated_data[generated_data['bnpl_active_count'] >= 3]) > 10:
            assert high_bnpl > low_bnpl, f'High BNPL count ({high_bnpl:.0f}) should have higher limits than low ({low_bnpl:.0f})'
...
class TestBehavioralRealism:
    ...
    def test_round_number_distribution(self, generated_data):
        ...
        assert round_1k_rate > 0.55, (
            f'Round $1K rate {round_1k_rate:.1%} too low (expect >55%)'
        )

    def test_new_behavioral_columns_exist(self, generated_data):
        """All 6 new behavioral columns should be present."""
        expected = [
            'application_channel', 'optimism_bias_flag',
            'financial_literacy_score', 'prepayment_buffer_months',
            'negative_equity_flag', 'loan_trigger_event',
        ]
        for col in expected:
    ...
    def test_optimism_bias_rate_near_target(self, generated_data):
        ...
        assert 0.15 <= bias_rate <= 0.40, (
            f'Optimism bias rate {bias_rate:.2%} outside expected 15-40% range'
        )
    ...
    def test_application_channel_valid_values(self, generated_data):
        ...
        assert actual.issubset(valid), (
            f'Invalid application channels: {actual - valid}'
        )

    def test_loan_trigger_event_not_empty(self, generated_data):
        """All records should have a non-empty loan trigger event."""
        assert (generated_data['loan_trigger_event'] != '').all(), (
            'Some loan_trigger_event values are empty'
        )

    def test_prepayment_buffer_range(self, generated_data):
        """Prepayment buffer months should be in valid range."""
        assert generated_data['prepayment_buffer_months'].min() >= 0, (
            'Prepayment buffer cannot be negative'
        )
        assert generated_data['prepayment_buffer_months'].max() <= 60, (
            'Prepayment buffer exceeds 60 months cap'
        )
    ...
    def test_gambling_tiered_distribution(self, generated_data):
        ...
        assert 0.05 <= has_gambling <= 0.35, (
            f'Gambling prevalence {has_gambling:.1%} outside expected range'
        )
    ...
    def test_home_loans_mostly_via_broker(self, generated_data):
        ...
        if len(home_loans) > 50:
            ...
            assert broker_rate > 0.40, (
                f'Home loan broker rate {broker_rate:.1%} too low (expect >40%)'
            )
