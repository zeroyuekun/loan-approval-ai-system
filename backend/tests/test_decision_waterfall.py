...
class DecisionWaterfallTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = CustomUser.objects.create_user(
            username='waterfall_test', password='TestPass123!',
            email='waterfall@test.com', role='customer',
        )
    ...
    def _run_pipeline_with_mocks(self, application, prediction_result, email_result, bias_result):
        ...
        mock_fraud.run_checks.return_value = {
            'passed': True, 'risk_score': 0.1, 'checks': [], 'flagged_reasons': [],
        }
        ...
        with patch('apps.agents.services.orchestrator.ModelPredictor', return_value=mock_predictor), \
             ...
             patch.object(PipelineOrchestrator, '_run_nbo_and_marketing_pipeline',
    ...
    def test_bankruptcy_flag_produces_policy_fail(self):
        ...
        decision = LoanDecision.objects.get(application=app)
        bankruptcy_entries = [
            e for e in decision.decision_waterfall
    ...
    def test_high_dti_produces_policy_fail(self):
        ...
        dti_entries = [
            e for e in decision.decision_waterfall
            if e['step'] == 'policy_rules' and e['reason_code'] == 'DTI_EXCEEDED'
        ]
...
class WaterfallEntryHelperTestCase(TestCase):
    ...
    def test_entry_has_correct_keys(self):
        entry = PipelineOrchestrator._waterfall_entry(
            'fraud_check', 'pass', 'FRAUD_CLEAR', 'No fraud indicators detected',
        )
    ...
    def test_timestamp_is_iso_format(self):
        entry = PipelineOrchestrator._waterfall_entry(
            'policy_rules', 'fail', 'DTI_EXCEEDED', 'DTI too high',
        )
