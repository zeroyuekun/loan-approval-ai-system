...
def _make_decision(decision='approved', confidence=0.92, risk_score=0.15,
                   model_version='v3.1', shap_values=None):
...
def _make_application(decision_kwargs=None, credit_score=720, loan_amount=350000,
                      loan_term_months=360, purpose='home_purchase'):
...
class TestPDFGeneration:
    ...
    def test_denied_generates_valid_pdf(self):
        app = _make_application(decision_kwargs={
            'decision': 'denied',
            'confidence': 0.85,
            'risk_score': 0.72,
            'shap_values': {
    ...
    def test_denied_pdf_larger_than_approved(self):
        ...
        denied = _make_application(decision_kwargs={
            'decision': 'denied',
            'shap_values': {
                'credit_score': -0.35,
                'annual_income': -0.25,
    ...
    def test_source_code_has_no_apology_language(self):
        ...
        for word in self.PROHIBITED_WORDS:
            assert word not in source_lower, (
                f'Prohibited word "{word}" found in PDF generator source code'
            )
...
class TestRiskGrading:
    ...
    @pytest.mark.parametrize('risk_score,expected_min_size', [
        (0.10, 500),   # AAA
        (0.25, 500),   # AA
        (0.40, 500),   # A
        (0.55, 500),   # BBB
