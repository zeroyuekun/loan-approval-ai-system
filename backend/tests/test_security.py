...
class TestPIIEncryption(TestCase):
    ...
    def test_encrypted_fields_not_plaintext_in_db(self):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                'SELECT primary_id_number FROM accounts_customerprofile WHERE user_id = %s',
                [self.user.pk]
            )
            raw_value = cursor.fetchone()[0]

        assert raw_value != 'ABC123456', (
            'Raw DB value should be encrypted, not plaintext'
        )
        # Fernet tokens start with 'gAAAAA'
        assert raw_value.startswith('gAAAAA'), (
            f'Expected Fernet token prefix, got: {raw_value[:20]}'
        )
    ...
    def test_secondary_id_encrypted(self):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                'SELECT secondary_id_number FROM accounts_customerprofile WHERE user_id = %s',
                [self.user.pk]
            )
            raw_value = cursor.fetchone()[0]
