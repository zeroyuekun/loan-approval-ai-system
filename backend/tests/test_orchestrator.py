...
def test_email_generation_failure(sample_application, orch_mocks):
    ...
    orch_mocks['predictor'].return_value.predict.return_value = _prediction('approved', 0.85, orch_mocks['model_version_id'])
...
def test_stale_pipeline_resets(sample_application, orch_mocks):
    ...
    LoanApplication.objects.filter(pk=sample_application.pk).update(
