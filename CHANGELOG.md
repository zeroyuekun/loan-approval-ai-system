...
- **#125 — Parse live Claude denial shape.** Live Claude denials were silently degrading to plain-text rendering in Gmail because the parser was tuned to fixture-shape templates. Two parsing gaps: (1) factor lines from the live prompt arrive prefixed with a bullet (`• Label: value`) rather than bare, so `_extract_factor_paragraphs` matched nothing; (2) the free-credit-report block arrives with a prose intro ("You can request a free copy...") instead of an explicit header, so `_extract_free_credit_report_block` fell through. This PR accepts optional bullet prefixes in factor matching, introduces two-branch credit-block detection (explicit header OR prose intro + ≥2 bureau bullets for Equifax / Illion / Experian), renames `_extract_free_credit_report_block` → `_extract_credit_report_block` to reflect the broader detection, mirrors the same changes byte-identically in `emailHtmlRenderer.ts`, and adds a `denial_06_live_shape` fixture + snapshot capturing the live layout end-to-end. Py↔TS parity preserved across all 16 fixtures; 58/58 backend + 22/22 frontend tests pass.
- **#126 — Aesthetic v3 polish.** Small, presentation-layer refinements on top of v2: denial hero drops the orange info icon Gmail was rendering as a loud block between header and body; dedicated marketing-close renderer so sign-off paragraphs sit on the correct 12px/2px rhythm; new `_next_nonblank_matches` helper + regex constants so bullet/numbered lookahead spacing no longer collapses across blank lines; ABN/Website special case trimmed in favour of margin-based spacing (Gmail quoting consistency); `<span class="badge">` treatment in header. Parser logic unchanged — this is purely the visual pass. Denial hero test flipped to assert the icon is absent. All 16 snapshots regenerated byte-identically across trees.
- **#124 — Staff re-run escalation.** The orchestrate-pipeline mutation short-circuits with `{status: "already_completed"}` when a completed `AgentRun` already exists for a loan — but staff clicking "Re-run AI Pipeline" on the application detail page expect a fresh run (+ new email), not a silent no-op. `usePipelineOrchestration` now escalates: after `orchestrate.mutateAsync()` returns `already_completed`, it immediately calls `forceRerun.mutateAsync()` with a staff-originated reason string. The dashboard is staff-only, so this path is never reached by customers. Adds 2 new test cases (positive escalation + force-rerun-failure branch) on top of the existing 8, for 10/10 green.
...
- HTML-escape of LLM-interpolated values in `emailHtmlRenderer.ts` / `html_renderer.py` (deferred from v1.10.3 and v1.10.4) remains slated for a future coordinated PR. The v1.10.5 parser change is additive (no existing fixture re-renders) so it does not disturb the 15 legacy parity snapshots.
