...
- **In prompts to the email LLM:** the email generator sends only anonymised feature summaries — no raw PII is sent to the LLM. The only identifiers in a prompt are internal `Application.id` and model scores. See `backend/apps/email_engine/services/prompts.py` for the enforced prompt shape. The backend is selectable (`EMAIL_LLM_BACKEND`): the default is Claude, with an optional free Groq backend chosen specifically because its free tier does not train on prompts (ADR 010); free tiers that train on prompts are excluded. Cross-border disclosure (APP 8) is audited per call in `APICallLog` (`provider`, `destination_country`).
