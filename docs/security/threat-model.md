...
| **I**nfo disclosure | PII leak via logs, error messages, or tracing | Customer PII | Sentry PII scrubber on by default; `pii_masking` utility on log lines; CSP; no stack traces in production responses | `backend/utils/pii_masking.py`, `config/settings/production.py`, `test_pii_masking.py`, `test_csp_headers.py` |
