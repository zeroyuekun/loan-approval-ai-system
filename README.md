...
- **XGBoost lift over a simple scorecard is measured, not assumed.** Every training run fits a logistic-regression baseline on `credit_score, annual_income, loan_amount, debt_to_income` and records `training_metadata.baseline_auc` plus `xgb_lift_over_baseline` so the main model's value-add is a specific number, not marketing copy. Latest run: **+0.1015 AUC lift** over a 3-feature scorecard on the same split. See [backend/docs/BASELINE_LIFT.md](backend/docs/BASELINE_LIFT.md) for the full writeup.
